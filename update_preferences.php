...
while ($row = mysqli_fetch_array($car_results, MYSQLI_ASSOC)){
	$time_results = mysqli_query($users_con, "select carpool_times_morning as t1, carpool_times_evening as t2 from preferences where id = (select max(id) from preferences where user_id = $userid)");
